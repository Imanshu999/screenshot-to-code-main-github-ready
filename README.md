# screenshot-to-code — Free / Self-Hosted
...
This repository is intended to be usable as a self-hosted, open-source application. There is no in-app subscription requirement in this build. You can run it locally and modify the source code freely according to the repository license.

**Important:** making the application free does not make third-party AI inference free. OpenAI, Anthropic, Gemini, Replicate, or other hosted providers may charge for their APIs. For a zero-API-cost setup, use a compatible local model/server such as an OpenAI-compatible local endpoint.

Never commit real API keys. Copy `backend/.env.example` to `backend/.env` and keep that file local.
